...
foreach ($user in $users) {
    ...
    If ($checkForUser -eq $false) {
        ...
        New-ADUser -Name $name -Path $ParentOU -SamAccountName $samAccountName -UserPrincipalName $UserPrincipalName -AccountPassword (ConvertTo-SecureString $password -AsPlainText -Force) -ChangePasswordAtLogon $false -Enabled $true -EmailAddress $UserPrincipalName
